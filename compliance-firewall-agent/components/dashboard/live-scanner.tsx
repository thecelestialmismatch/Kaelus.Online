  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {EXAMPLE_TEXTS.map((ex, i) => (
          <button
            ...
            className="text-[11px] px-3 py-1.5 rounded-lg border border-white/10 text-slate-400 hover:text-slate-300 hover:border-white/15 transition-all"
          >
            Try: {ex.label}
          </button>
        ))}
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-4 border-b border-white/10">
          ...
          <textarea
            ...
            rows={6}
            className="w-full bg-transparent text-sm text-slate-200 placeholder-white/20 focus:outline-none resize-none font-mono leading-relaxed"
          />
        </div>
          ...
          <div className={`glass-card p-5 border ${colors.border}`}>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-400">Entities Found</p>
              </div>
              <div className="text-center">
            ...
            <div className="glass-card p-5">
              ...
              <div className="space-y-2">
                {result.entities.map((entity, i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-white/[0.03] border border-white/10"
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-mono text-brand-500 bg-brand-500/10 px-2 py-0.5 rounded">
                        {entity.type}
                      </span>
                      <span className="text-xs text-slate-400 font-mono">{entity.value_redacted}</span>
                    </div>
          ...
          {result.classifications.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {result.classifications.map((c, i) => (
                <span
                  key={i}
                  className="text-[10px] font-medium px-2.5 py-1 rounded-full bg-brand-500/10 text-brand-500 border border-brand-200"
                >
                  {c}
                </span>
              ))}
      ...
      {scanHistory.length > 0 && (
        <div>
          <h3 className="text-xs font-medium text-slate-400 mb-3">Recent Scans</h3>
            ...
            {scanHistory.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[11px] text-slate-400 flex-1 truncate">{item.text}...</span>
